...
pnpm check:integrate     # G2.2 sqlite integration; in-memory, postgres, or a stubbed 201 fails
...
runs the G2.8 license whitelist, the G2.7 migrate check, and the G2.2 sqlite integration
on the same events; it does not skip, filter, or `continue-on-error` the L1 suite.
`check:migrate` and `check:integrate` are not folded into `verify`.
