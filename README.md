...
| `packages/quality` | G2.8 license whitelist, G1.5 coverage gate, and G2.4 Semgrep + CodeQL |
...
the G2.6 artifact budget, and the G2.4 Semgrep + CodeQL SAST checks on the same events; it does
not skip, filter, or `continue-on-error` the L1 suite. `check:migrate`, `check:integration`,
`check:artifact`, `check:sast`, and `check:codeql` are not folded into `verify`.
