...
runs the G2.8 license whitelist, the G2.7 migrate check, the G2.2 sqlite integration check, and
the G2.6 artifact budget on the same events; it does not skip, filter, or `continue-on-error` the
L1 suite. `check:migrate`, `check:integration`, and `check:artifact` are not folded into `verify`.
