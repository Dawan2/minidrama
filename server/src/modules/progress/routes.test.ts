 ...
 * resolver accepts only tokens its own session store issued, which is the subject of the first group
 * below rather than an obstacle to the rest. `tok_a` is `user_a` and `tok_b` is `user_b`; nothing
 * else resolves.
 */
