...
import type { FavoritesCursor, FavoritesPage, FavoritesStore } from './favorites.js';
...
describe('createInMemoryFavoritesStore', () => {
  ...
  describe('list', () => {
    it('answers an empty page for a viewer who follows nothing', async () => {
      ...
      expect(await store.list('user_a', { limit: 20 })).toEqual({ rows: [], hasMore: false });
    });
    ...
    it('returns most recently followed first', async () => {
      ...
      await store.add('user_a', 'drm_1', NOW);
      await store.add('user_a', 'drm_2', NOW + 60_000);
      await store.add('user_a', 'drm_3', NOW + 30_000);

      const page = await store.list('user_a', { limit: 20 });
    ...
    it('orders by the stored timestamp, not by insertion', async () => {
      ...
      await store.add('user_a', 'drm_2', NOW + 60_000);
      await store.remove('user_a', 'drm_1');
      await store.add('user_a', 'drm_1', NOW + 120_000);

      expect((await store.list('user_a', { limit: 20 })).rows.map((row) => row.dramaId)).toEqual([
        'drm_1',
        'drm_2',
      ]);
    });
    ...
    it('breaks a tied timestamp on the drama id, descending', async () => {
      ...
      await store.add('user_a', 'drm_b', NOW);
      await store.add('user_a', 'drm_a', NOW);
      await store.add('user_a', 'drm_c', NOW);
    ...
    it('never lists another viewer’s favourites', async () => {
      ...
      await store.add('user_a', 'drm_1', NOW);
      await store.add('user_b', 'drm_2', NOW + 1);
    ...
    describe('paging', () => {
      async function seed(count: number): Promise<FavoritesStore> {
        ...
        for (let index = 0; index < count; index += 1) {
          ...
          await store.add('user_a', `drm_${String(index).padStart(2, '0')}`, NOW + index);
        }
        return store;
      }
      ...
      it('reports there is more, and takes a cursor for the rest', async () => {
        ...
        const first = await store.list('user_a', { limit: 2 });
        expect(first.rows.map((row) => row.dramaId)).toEqual(['drm_04', 'drm_03']);
        expect(first.hasMore).toBe(true);

        const second = await store.list('user_a', { limit: 2, after: cursorOf(first) });
        expect(second.rows.map((row) => row.dramaId)).toEqual(['drm_02', 'drm_01']);
        expect(second.hasMore).toBe(true);

        const third = await store.list('user_a', { limit: 2, after: cursorOf(second) });
      ...
      it('pages through favourites recorded in the same millisecond', async () => {
        ...
        for (const dramaId of ['drm_a', 'drm_b', 'drm_c', 'drm_d']) {
          await store.add('user_a', dramaId, NOW);
        }
      ...
      it('still pages when the row the cursor names has been unfollowed', async () => {
        ...
        await store.remove('user_a', 'drm_03');

        expect(
          (await store.list('user_a', { limit: 2, after })).rows.map((row) => row.dramaId),
      ...
      it('does not show a newly added favourite on a later page', async () => {
        ...
        await store.add('user_a', 'drm_new', NOW + 1_000);

        expect(
          (await store.list('user_a', { limit: 2, after: cursorOf(first) })).rows.map(
            (row) => row.dramaId,
          ),
        ).toEqual(['drm_00']);
      });
      ...
      it('answers an empty page for a cursor past the end of the list', async () => {
        ...
        const page = await store.list('user_a', {
          limit: 2,
