...
> Merged forward onto **`51ab72f`** (leftover C6 INF-004 S-C3 and after-lock QA-010
> SCR-02 landed while this slot wrote).
...
> against `51ab72f` rather than copied from a handoff. PLY-011 landed on `main` **after**
...
> are ancestors of `51ab72f` before this file merges.
...
| **3** | **QA-010 remainder** after SCR-02 | **In flight** (`bc-25aea5c1`, next a11y screen). Leave its files. Do not start a twin HOME fixture. SCR-13 and SCR-02 are already on `main` |
...
| `bc-25aea5c1` (W20 work next a11y screen) | **RUNNING.** No `cursor/*` branch visible on origin yet | Do not guess its files. Re-derive `origin/main` before picking an a11y remainder. Do not start a twin of the next screen |
...
Verified at `51ab72f`. The report file is an ancestor (`e6926f2` merge / `bd04816`
unique). Where §§0–12 talk about `eff5eb2` and §13 talks about `9b563b0`, HEAD is that
report plus PLY-011 plus leftover C6 S-C3 plus after-lock SCR-02. Where a handoff
...
| Protocol C4 exit (`wave-protocol.md` §5.1) | C6 plan (`aa8a9e8`) | C6 report (`eff5eb2` / `9b563b0`) | This tree (`51ab72f`) |
|---|---|---|---|
...
| a11y 门禁上线且核心屏零 critical/serious | **Not met.** QA-010 in flight. No axe-core job | **Still not met as S-A1 on every SCR/PNL.** Smallest L1 job on `main` (`check:a11y`, SCR-13 fixture only, host=jsdom). Dated notes say this slice does **not** close exit 3 | **Still not met as S-A1.** Two committed fixtures: `scr-13-fallback.html` and `scr-02-home.html`. Dated notes say SCR-02 still does not close exit 3. Next screen is **in flight** (`bc-25aea5c1`) |
...
| ID | C6 plan (`aa8a9e8`) | C6 report (`eff5eb2` / §13) | This tree (`51ab72f`) |
|---|---|---|---|
| **D-17** rank 1 | Open. Cannot be code-fixed | **Open. Unchanged** | **Open. Unchanged.** CI run [33130979543](https://github.com/Dawan2/minidrama/actions/runs/33130979543) on `51ab72f` (2026-08-28T00:50:23Z): 5s, empty `steps`. Annotation on check-run `98720176331`: spending-limit / failed payments. Last successful CI on `main` is still [33112204165](https://github.com/Dawan2/minidrama/actions/runs/33112204165) at PNL-01, 2026-08-27T20:12:42Z |
...
| **QA-010** rank 3 | In flight (`bc-b0108787`) | **Closed as the smallest L1 job** (`a68a964` / `3e8bdb2`). Remainder: other implemented screens | **Smallest job + SCR-02 on `main`.** Remainder after SCR-02 **in flight** (`bc-25aea5c1`). Rank 3 is that remainder, not a retake of SCR-13 or SCR-02 |
...
The C6 plan's rank-1 item cannot land from a branch. Rank 3's *job* and *SCR-02 stem*
landed; the *further-screen remainder* is in flight. Rank 2's sheet is closer (PLY-011
included) and still not 全过. INF-004 S-C3 landed while this slot wrote. That is the
scorecard, and it is why C7 is still remediation rather than a new epic.

### 1.3 Defects the C6 report left, re-tested here

| ID | Sev | State at `51ab72f` |
|---|:---:|---|
...
33130979543  failure  51ab72f  CI   5s  2026-08-28T00:50:23Z
33130979597  failure  51ab72f  L2   6s  2026-08-28T00:50:23Z

$ gh api repos/Dawan2/minidrama/check-runs/98720176331/annotations
...
| **Gated on** | Re-verification of running-count C6 / C2 L2 observer, or an explicit P3 amendment of §4.3. Neither has happened. a11y remainder is rank 3 (in flight `bc-25aea5c1`) |
...
against `51ab72f`, the landed halves (including PLY-011) are not retaken, and implement
waves are told not to invent `playbackRate`. A PR that lands 倍速 as "C7 leftover" fails
this rank. Leave `bc-7fbe0bc1`.

---

### Rank 3 — QA-010 remainder: after SCR-02, in flight

| | |
|---|---|
| **Owner** | Work slot C, then A for screen fixes. **This wave: `bc-25aea5c1` already started** (next a11y screen) |
| **Existing ID** | **QA-010**. Smallest job and SCR-02 already on `main`. Remainder is S-A1 on the other implemented screens |
...
named L1 step. Host is jsdom, not TikTok WebView. Two committed fixtures:
`scr-13-fallback.html` and `scr-02-home.html` (`REQUIRED_SCREEN_STEMS`). Dated notes say
SCR-02 still does **not** close protocol-C4 exit 3.

After-lock unique commit `35fa8e8` required `scr-02-home.html` next to SCR-13. That stem
**landed** at `51ab72f` while this slot wrote. Rank 3 is now the *next* implemented
screen, and `bc-25aea5c1` already started it.

**What this task is not.** A docs retake of QA-011. A retake of SCR-13 or SCR-02. A
`continue-on-error` axe job. A scan of screens that do not exist (`#/recharge`, `#/vip`).
A claim that jsdom is TikTok WebView.

**What a C7 work slot may do.** Nothing until `bc-25aea5c1` is idle and `origin/main` is
re-derived. If that agent lands a third stem, this rank's next remainder is the *other*
...
| **QA-010 smallest job / SCR-13** | On `main`. Rank 3 is the other screens after SCR-02. Do not retake `scr-13-fallback.html` |
...
`bc-25aea5c1`) and rank 2's in-flight remaining C4 playback (`bc-7fbe0bc1`) and the
blocked C4-03 / C4-07 rows, there is no unblocked engineering item that is not already
owned, gated, or P3's file.
...
§6.2 still describes a tree that is not `51ab72f` ("no database, no migration, 27 seed
...
Leave `cursor/w18-work-c6-left-72c4` and `cursor/w18-work-c6-after-lock-72c4` as
ancestors — they landed. Leave `bc-25aea5c1` and `bc-7fbe0bc1` until those agents are
idle. They are live remainders, not drop-or-archive leftovers.
...
Re-derived at `51ab72f`. Commands to re-run, not to believe.
...
# scr-02-home.html  scr-13-fallback.html
...
Rank 3 — QA-010 remainder after SCR-02. In flight `bc-25aea5c1` (next a11y screen).
  Leave it. Smallest job and SCR-02 already on main. Do not retake those fixtures.
...
SCR-02, INF-004 S-C1, INF-004 S-C3, PRG-001 named case, PRG-002 drama CTA, C4-01,
C4-04, PLY-010 landed slices, PLY-012, PLY-011 S6 lock chrome, S7 stall.

Tier A paper
  C5-03 = D-19 P3 writeback of wave-protocol.md §6.2
  Integrator: drop leftover remain / c4-subseq. Leave the two in-flight W20 tips.
...
**First C7 implement picks.** Re-derive `origin/main` first. Leave `bc-25aea5c1` and
`bc-7fbe0bc1`. Do not pick D-17, G2.3, HOME continue, leftover ads, leftover remain,
`#/vip`, Postgres-as-sqlite, 倍速, PLY-012, PLY-011 `locked-chrome.tsx`, QA-010 SCR-13,
QA-010 SCR-02, INF-004 S-C1, or INF-004 S-C3. If those two agents are idle and did not
land, pick QA-010 remainder or remaining C4 playback against the tree at that time —
...
- **Opening protocol-C4 播放体验 as a new epic.** Ranked, split, not scheduled by this
  ...
  playback (`bc-7fbe0bc1`); another is on the next a11y screen (`bc-25aea5c1`).
- **A 倍速 / `playbackRate` client control.** X-26. Forbidden as a leftover by the C5
  plan, the C5 report, the C6 plan, and the C6 report.
- **A twin QA-010 remainder.** In flight `bc-25aea5c1`. **A twin INF-004 S-C3.** Landed.
  **A second S6.** Leftover unique files shipped S-C3, not a twin chrome.
...
|---|---|---|
| 2026-08-28 | W20 · plan | First version. Absorbed `docs/verify/cycle-6-report.md` at `e6926f2` (not passed; protocol-C4 still 1/3; D-17 still billing-red). Cut from `e6926f2`; merged forward onto `51ab72f`. Leftover C6 `bc-2c841f7a` (INF-004 S-C3) and after-lock `bc-1a5a2455` (QA-010 SCR-02) **landed while this slot wrote**. Ranked: D-17 (cannot be code-fixed), protocol-C4 交互验收单 still 1/3, QA-010 remainder after SCR-02 in flight (`bc-25aea5c1`), C4-03 still open (do not fake), C4-07 still blocked. Remaining C4 playback in flight (`bc-7fbe0bc1`). Did not invent AM answers, Beans, EIS, BytePlus, Postgres, VIP, or a CI-restoration date. Did not implement product code. Did not open protocol-C5 变现闭环 |
