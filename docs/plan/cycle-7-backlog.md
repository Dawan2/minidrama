...
> against `e6926f2` rather than copied from a handoff. PLY-011 landed on `main` **after**
...
> touched. The only files added are this one and `docs/handoff/w20-plan.md`. No pull
> request.
...
| **2** | **Protocol-C4 交互验收单** still 1/3 | Do not open as an implement epic. Named remainders already on `main` (§1.2). 倍速 / scrub stay X-26. Do not invent `playbackRate`. Do not retake landed chrome |
| **3** | **QA-010 remainder** other implemented screens | **In flight** (`bc-1a5a2455`, after-lock). Leave its files. Do not start a twin SCR-02 fixture. Smallest job (SCR-13) is already on `main` |
| **4** | **INF-004 remainder** S-C3 echo-only | **In flight leftover C6** (`bc-2c841f7a`). Leave its files. Do not start a twin audit. S-C1 is already on `main` |
| **5** | **C4-03** T14 / T16 / T15 | Still open. **Do not fake.** May defer with IDs kept |
| **6** | **C4-07** SCR-11 / D8 | Still blocked. No subscription contract. Do not invent `/v1/subscriptions` or `#/vip` |
...
|---|---|---|
| `bc-2c841f7a` (W18 work leftover C6 item) | **RUNNING.** `origin/cursor/w18-work-c6-left-72c4` unique commit `dea3066` `feat(INF-004): fail CI self-audit on echo-only workflow steps`, parent `9b563b0`. Not an ancestor of `e6926f2` | Leave `packages/quality/src/audit.ts` / `check-audit.ts` / `.github/workflows/ci.yml`. Do not start a twin S-C3. The C6 report §13 warned this agent was a twin PLY-011 (`LockedCover.tsx` / `player-lock.ts`, SHA `af1b7ff`). **Re-derived:** that SHA is not in this clone; leftover unique files are now the INF-004 remainder, and the landed S6 (`locked-chrome.tsx`) is an ancestor. Do not merge leftover as a second S6. Do not retake `locked-chrome.tsx` |
| `bc-1a5a2455` (W18 work next C6 after lock chrome) | **RUNNING.** Started after the C6-report lock. `origin/cursor/w18-work-c6-after-lock-72c4` unique commit `35fa8e8` `ci(qa-010): require the SCR-02 home fixture next to SCR-13`, parent `9b563b0`. Not an ancestor of `e6926f2` | Leave `packages/quality/a11y/` and `packages/quality/src/a11y.ts`. Do not start a twin HOME fixture. Do not claim protocol-C4 exit 3 closed if it lands — dated notes already say SCR-13 is not S-A1 on every screen |
...
Both in-flight tips also touch `docs/14-quality-gates.md`. That collision is registered in
§6 (`X-27`). This slot does not edit that file.

---

## 1. What the C6 report said, and what `main` is now

Verified at `e6926f2`. The report file is this SHA (merge) / `bd04816` (unique). Where
§§0–12 talk about `eff5eb2` and §13 talks about `9b563b0`, HEAD is that report plus the
already-landed PLY-011 merge. Where a handoff disagrees, the tree wins.
...
| Protocol C4 exit (`wave-protocol.md` §5.1) | C6 plan (`aa8a9e8`) | C6 report (`eff5eb2` / `9b563b0`) | This tree (`e6926f2`) |
|---|---|---|---|
...
| a11y 门禁上线且核心屏零 critical/serious | **Not met.** QA-010 in flight. No axe-core job | **Still not met as S-A1 on every SCR/PNL.** Smallest L1 job on `main` (`check:a11y`, SCR-13 fixture only, host=jsdom). Dated notes say this slice does **not** close exit 3 | **Still not met as S-A1.** One committed fixture, `packages/quality/a11y/screens/scr-13-fallback.html`. After-lock is adding SCR-02 **in flight**, not on this SHA |
...
| ID | C6 plan (`aa8a9e8`) | C6 report (`eff5eb2` / §13) | This tree (`e6926f2`) |
|---|---|---|---|
| **D-17** rank 1 | Open. Cannot be code-fixed | **Open. Unchanged** | **Open. Unchanged.** CI run [33130563770](https://github.com/Dawan2/minidrama/actions/runs/33130563770) on `e6926f2` (2026-08-28T00:42:22Z): 5s, empty `steps`. Annotation on check-run `98718847762`: spending-limit / failed payments. Last successful CI on `main` is still [33112204165](https://github.com/Dawan2/minidrama/actions/runs/33112204165) at PNL-01, 2026-08-27T20:12:42Z |
| **Protocol-C4** rank 2 | 1/3. Sheet not full. Do not invent `playbackRate` | **1/3.** Exit 2 stays met. Exit 1: plugin-owned 倍速 / scrub kept, stall, PLY-011 on postscript. Exit 3: job online as SCR-13 only | **1/3.** Same. PLY-011 is an ancestor. Sheet still not 全过 |
| **QA-010** rank 3 | In flight (`bc-b0108787`) | **Closed as the smallest L1 job** (`a68a964` / `3e8bdb2`). Remainder: other implemented screens | **Smallest job on `main`.** Remainder **in flight** (`bc-1a5a2455` / SCR-02). Rank 3 is that remainder, not a retake of SCR-13 |
| **PLY-012** | Closed as merge (`aa8a9e8`) | **Stays closed** | **Stays closed.** Do not retake `player-fatal.ts` |
| **C4-03** rank 4 | Open. Do not fake | **Open. Not faked** | **Open. Not faked.** `postgres:` refused. No Drizzle. Redis only in refuse tests. Rank 5 here because two in-flight remainders sit above it |
| **C4-07** rank 5 | Open, blocked | **Open, blocked** | **Open, blocked.** No `#/vip`. No OpenAPI subscription path. Rank 6 here |
| **INF-004 S-C1** | Not a ranked remainder | **On `main`** (`f40fe36`). Reverse-verified. S-C3 / S-C4 left as further slices | **S-C1 on `main`.** S-C3 echo-only **in flight** (`bc-2c841f7a` / `dea3066`). Rank 4. Do not retake S-C1 |
...
The C6 plan's rank-1 item cannot land from a branch. Rank 3's *job* landed; its
*remainder* is in flight. Rank 2's sheet is closer (PLY-011 included) and still not
全过. That is the scorecard, and it is why C7 is still remediation rather than a new
epic.

### 1.3 Defects the C6 report left, re-tested here

| ID | Sev | State at `e6926f2` |
|---|:---:|---|
...
33130563770  failure  e6926f2  CI   5s  2026-08-28T00:42:22Z
33130563704  failure  e6926f2  L2   6s  2026-08-28T00:42:22Z

$ gh api repos/Dawan2/minidrama/check-runs/98718847762/annotations
...
| **Gated on** | Re-verification of running-count C6 / C2 L2 observer, or an explicit P3 amendment of §4.3. Neither has happened. a11y remainder is rank 3 (in flight) |
...
against `e6926f2`, the landed halves (including PLY-011) are not retaken, and implement
waves are told not to invent `playbackRate`. A PR that lands 倍速 as "C7 leftover" fails
this rank.

---

### Rank 3 — QA-010 remainder: other implemented screens, in flight

| | |
|---|---|
| **Owner** | Work slot C, then A for screen fixes. **This wave: `bc-1a5a2455` already started** (after-lock) |
| **Existing ID** | **QA-010**. Smallest job already on `main`. Remainder is S-A1 on the other implemented screens |
...
named L1 step. Host is jsdom, not TikTok WebView. One committed fixture:
`packages/quality/a11y/screens/scr-13-fallback.html`. Dated notes say this slice does
**not** close protocol-C4 exit 3.

After-lock unique commit `35fa8e8` requires `scr-02-home.html` next to SCR-13. That is
the remainder, in flight, not on this SHA.

**What this task is not.** A docs retake of QA-011. A retake of SCR-13. A
`continue-on-error` axe job. A scan of screens that do not exist (`#/recharge`, `#/vip`).
A claim that jsdom is TikTok WebView.

**What a C7 work slot may do.** Nothing until `bc-1a5a2455` is idle and `origin/main` is
re-derived. If that agent lands SCR-02, this rank's next remainder is the *other*
implemented screens against the tree at that time — not against this paragraph's guess.
If it does not land, the next slot picks QA-010 remainder against the tree at that time.
...
### Rank 4 — INF-004 remainder: S-C3 echo-only, in flight leftover C6

| | |
|---|---|
| **Owner** | Work slot C. **This wave: `bc-2c841f7a` already started** (leftover C6) |
| **Existing ID** | **INF-004**. S-C1 (`continue-on-error` / `if: false` / swallowed exits) already on `main` (`f40fe36`). W19 reverse-verified it. Remainder named in `docs/14-quality-gates.md` §2.1: S-C3 echo-only steps, S-C4 required-checks vs branch protection, job-count ratchet |
| **Gated on** | Nothing external |

Leftover unique commit `dea3066` fails CI self-audit on echo-only workflow steps. Files:
`packages/quality/src/audit.ts`, `audit.test.ts`, `cli/check-audit.ts`,
`.github/workflows/ci.yml`, `docs/14-quality-gates.md`. Not on this SHA.

**What this task is not.** A retake of S-C1. A twin of `bc-2c841f7a`. A second S6 (see
§0.4). A `continue-on-error` key.

**What a C7 work slot may do.** Nothing until `bc-2c841f7a` is idle and `origin/main` is
re-derived. Do not start a twin echo-only detector.
...
| **QA-010 smallest job / SCR-13** | On `main`. Rank 3 is the other screens. Do not retake `scr-13-fallback.html` |
| **INF-004 S-C1** | On `main`. Rank 4 is S-C3. Do not retake the continue-on-error detector |
...
and rank 4 (INF-004 remainder in flight) and the blocked C4-03 / C4-07 rows, there is no
...
§6.2 still describes a tree that is not `e6926f2` ("no database, no migration, 27 seed
...
Leave `cursor/w18-work-c6-left-72c4` and `cursor/w18-work-c6-after-lock-72c4` until those
agents are idle. They are live remainders, not drop-or-archive leftovers.
...
Re-derived at `e6926f2`. Commands to re-run, not to believe.
...
# scr-13-fallback.html   (only committed fixture on this SHA)
...
$ git log --oneline origin/main..origin/cursor/w18-work-c6-left-72c4
dea3066 feat(INF-004): fail CI self-audit on echo-only workflow steps

$ git log --oneline origin/main..origin/cursor/w18-work-c6-after-lock-72c4
35fa8e8 ci(qa-010): require the SCR-02 home fixture next to SCR-13
...
Rank 3 — QA-010 remainder screens. In flight `bc-1a5a2455` (after-lock, SCR-02).
  Leave it. Smallest job already on main. Do not retake scr-13-fallback.html.

Rank 4 — INF-004 S-C3 echo-only. In flight leftover C6 `bc-2c841f7a` (`dea3066`).
  Leave it. S-C1 already on main. Do not retake the continue-on-error detector.
  Do not merge leftover as a second S6.

Rank 5 — C4-03 T14/T16/T15. Do not fake; may defer with IDs kept.

Rank 6 — C4-07. A contract, then GATE-2 + GATE-4.

Not remaining: G2.3, HOME continue, C5-01, C5-02, QA-011, QA-010 SCR-13,
INF-004 S-C1, PRG-001 named case, PRG-002 drama CTA, C4-01, C4-04,
PLY-010 landed slices, PLY-012, PLY-011 S6 lock chrome, S7 stall.

Tier A paper
  C5-03 = D-19 P3 writeback of wave-protocol.md §6.2
  Integrator: drop leftover remain / c4-subseq. Leave the two in-flight C6 tips.
...
**First C7 implement picks.** Re-derive `origin/main` first. Leave `bc-2c841f7a` and
`bc-1a5a2455`. Do not pick D-17, G2.3, HOME continue, leftover ads, leftover remain,
`#/vip`, Postgres-as-sqlite, 倍速, PLY-012, PLY-011 `locked-chrome.tsx`, QA-010 SCR-13,
or INF-004 S-C1. If those two agents are idle and did not land, pick QA-010 remainder
or INF-004 S-C3 against the tree at that time — not against a guess written here.
...
- **Opening protocol-C4 播放体验 as a new epic.** Ranked, split, not scheduled by this
  slot. Named remainders are on `main`. One leftover C6 sibling and one after-lock
  sibling are already on the remaining a11y / INF-004 rows.
- **A 倍速 / `playbackRate` client control.** X-26. Forbidden as a leftover by the C5
  plan, the C5 report, the C6 plan, and the C6 report.
- **A twin QA-010 remainder.** In flight after-lock. **A twin INF-004 S-C3.** In flight
  leftover C6. **A second S6.** Leftover unique files are no longer that twin; do not
  recreate it.
...
| **X-27** | In-flight leftover C6 (`bc-2c841f7a`) and after-lock (`bc-1a5a2455`) both edit `docs/14-quality-gates.md` | **Those two agents, then C** | Leave both. The later merge absorbs. This slot does not edit C's file |
...
|---|---|---|
| 2026-08-28 | W20 · plan | First version. Absorbed `docs/verify/cycle-6-report.md` at `e6926f2` (not passed; protocol-C4 still 1/3; D-17 still billing-red). Cut from `e6926f2`. QA-010 smallest job, INF-004 S-C1, kept 倍速 / scrub, S7 stall, PLY-012, PLY-011 S6 lock chrome **already on `main`**. Ranked: D-17 (cannot be code-fixed), protocol-C4 交互验收单 still 1/3, QA-010 remainder in flight after-lock (`bc-1a5a2455`), INF-004 S-C3 in flight leftover C6 (`bc-2c841f7a`), C4-03 still open (do not fake), C4-07 still blocked. Did not invent AM answers, Beans, EIS, BytePlus, Postgres, VIP, or a CI-restoration date. Did not implement product code. Did not open protocol-C5 变现闭环 |
