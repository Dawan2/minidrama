...
> The cut tip *is* `docs/verify/cycle-5-report.md` (W17 independent verify, not passed; D-17 still
...
> existing file was modified except to absorb `origin/main` after the PLY-012 merge.
...
Read `origin/main` (cut `0846582`, then `aa8a9e8`), `docs/verify/cycle-5-report.md`,
`docs/plan/cycle-5-backlog.md`, `docs/plan/wave-protocol.md` §4.3 / §5.1 / §7, and
...
double-tap are recorded as on `main`** rather than still the sheet, and how **PLY-012 is recorded
as landed** (`aa8a9e8` / `bc-264077b7`) rather than still in flight — it was RUNNING at first
draft and an ancestor of `main` before this file was merged. **QA-010** stays in flight
(`bc-b0108787`).
...
| Rank | Item | Disposition on `aa8a9e8` |
|---|---|---|
| **1** | **D-17** CI billing | **Open. Cannot be code-fixed.** Run 33128549734 on `aa8a9e8`: 4s, empty steps. Same spending-limit class as check-run `98711991675` on `0846582`. Last green CI still PNL-01 |
| **2** | **Protocol-C4 交互验收单** | **Still not met, despite PLY-010.** Swipe / playNext / double-tap on `main`. PLY-012 now on `main` too. 倍速 / scrub plugin-owned (X-26). Tap pause is VePlayer's. Sheet is not 全过. Do not invent `playbackRate` |
...
| **—** | **PLY-012** | **Not remaining.** Landed at `aa8a9e8`. Was in flight at first draft (`bc-264077b7`). Do not retake |
...
C5-01 / C5-02 / QA-011 / HOME continue / G2.3 / PRG-001 remainder / PRG-002 drama CTA / PLY-012
are **not remaining**.

First C6 implement instruction: re-derive `origin/main`. Leave `bc-b0108787`. There is no
unblocked engineering pick behind it that is not P3's file or a leftover-branch drop.
...
| P-03 | Rank the 交互验收单 separately from a11y | C5 lumped them as 0/3. The tree now has PLY-010 slices and PLY-012 on `main`, QA-010 in flight. One rank would invite a twin of the axe job |
...
| P-09 | Record PLY-012 as landed | `aa8a9e8` / `271408f`. First draft said in-flight; the tree moved. Do not retake `player-fatal.ts` |
...
**Implement.** Re-derive `origin/main` first: `bc-b0108787` (QA-010) is running. Leave its files.
Do not pick D-17, G2.3, HOME continue, leftover ads, leftover remain, `#/vip`,
sqlite-named-as-Postgres, 倍速, or PLY-012.
...
mock, start protocol-C5 变现闭环, or start a twin of the in-flight QA-010 agent.
...
that is a regression on the landed UI. If `player-fatal.ts` has disappeared, that is a regression
on PLY-012. If an `axe` job has appeared in `.github/workflows/`, rank 3 may have landed —
re-derive rather than retaking.
