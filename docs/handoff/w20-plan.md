...
> Merged forward onto `51ab72f` (leftover C6 INF-004 S-C3 and after-lock QA-010 SCR-02
...
Read `origin/main` (cut `e6926f2`, then `51ab72f`), `docs/verify/cycle-6-report.md`,
...
`9b563b0` and shipped S-C3. New in-flight W20 work: `bc-25aea5c1` (next a11y screen) and
`bc-7fbe0bc1` (remaining C4 playback).

---

## 2. Ranked remaining work (one screen)

| Rank | Item | Disposition on `51ab72f` |
|---|---|---|
| **1** | **D-17** CI billing | **Open. Cannot be code-fixed.** Run 33130979543 on `51ab72f`: 5s, empty steps. Annotation on check-run `98720176331` is the spending-limit sentence. Last green CI still PNL-01 |
...
| **3** | **QA-010 remainder** after SCR-02 | **In flight** (`bc-25aea5c1` next a11y screen). Smallest job and SCR-02 already on `main`. Leave it |
...
unmet). C5-01 / C5-02 / QA-011 / QA-010 SCR-13 / QA-010 SCR-02 / INF-004 S-C1 /
INF-004 S-C3 / HOME continue / G2.3 / PRG-001 remainder / PRG-002 drama CTA / PLY-012 /
PLY-011 / S7 stall / kept 倍速 / scrub are **not remaining**.

First C7 implement instruction: re-derive `origin/main`. Leave `bc-25aea5c1` and
`bc-7fbe0bc1`. There is no unblocked engineering pick behind them that is not P3's file
...
| P-04 | Record QA-010 smallest job, SCR-02, INF-004 S-C1, and INF-004 S-C3 as closed on this tree | W19 reverse-verified the first two. Leftover C6 and after-lock landed while this slot wrote |
...
| P-11 | Leave the two new in-flight W20 work tips | `bc-25aea5c1` next a11y; `bc-7fbe0bc1` remaining C4 playback. §8 rule 4 |
| P-12 | Record leftover C6 and after-lock as landed | First draft said in-flight (user-noted `bc-2c841f7a` / `bc-1a5a2455`). The tree moved. Do not retake their files |
...
**Implement.** Re-derive `origin/main` first: `bc-25aea5c1` (next a11y screen) and
`bc-7fbe0bc1` (remaining C4 playback) are running. Leave their files. Do not pick D-17,
G2.3, HOME continue, leftover ads, leftover remain, `#/vip`, sqlite-named-as-Postgres,
倍速, PLY-012, PLY-011 `locked-chrome.tsx`, QA-010 SCR-13, QA-010 SCR-02, INF-004 S-C1,
or INF-004 S-C3.
...
`[x]` off a mock, start protocol-C5 变现闭环, start a twin of either in-flight W20 agent,
or merge leftover C6 as a second S6.
...
has disappeared, that is a regression on after-lock. If `audit.ts` no longer fails
echo-only steps, that is a regression on leftover C6 S-C3. If a third a11y fixture has
appeared next to SCR-02 / SCR-13, rank 3 may have landed — re-derive rather than
retaking.
