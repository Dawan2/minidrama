...
│   └── quality/                # G2.8 licenses, G1.5 coverage, G1.9 commits, G1.10 skip/empty tests, G1.8 Gitleaks, G1.6 oasdiff, G2.4 Semgrep + CodeQL, G2.5 Trivy, G2.3 Playwright; not bundled
...
├── .github/workflows/ci.yml    # L1: G1.8 Gitleaks, G1.6 oasdiff, format, lint, types, G1.9 commits, G1.10 skip/empty tests, tests+coverage, build, guardrails
...
pnpm verify               # format:check + lint + typecheck + check:commits + check:skips + test:coverage + check:coverage + build + guardrails
...
pnpm check:commits        # G1.9; Conventional Commits on origin/main..HEAD; a prose subject fails
pnpm gen:minis-config     # regenerate app/minis.config.json from the domain registry
```

`pnpm verify` is the L1 suite minus G1.8 and G1.6, plus G1.9 and G1.10. L1 CI installs Gitleaks and
oasdiff, runs `check:secrets`, then `check:contract`, then format/lint/types, then
`check:commits`, then `check:skips`, then tests+coverage, then `gen:minis-config`. G1.8 and G1.6 are not folded
into `verify` because they need the binaries; a missing binary is red in CI, not a skip.
G1.9 and G1.10 are folded into `verify` because they have no extra binary. L2
