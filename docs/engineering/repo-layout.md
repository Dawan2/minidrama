...
│   └── quality/                # G2.8 licenses, G1.5 coverage, G2.4 Semgrep + CodeQL; not bundled
...
└── .github/workflows/l2.yml    # L2: G2.8 + G2.7 + G2.2 + G2.6 + G2.4 Semgrep + CodeQL; does not skip L1
```
...
`check:integration`, `check:artifact`, `check:sast`, `check:codeql`) is a second workflow and is not
