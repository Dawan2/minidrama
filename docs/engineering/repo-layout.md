...
│   └── quality/                # G2.8 licenses, G1.5 coverage, G1.9 Conventional Commits, G1.8 Gitleaks, G1.6 oasdiff, G2.4 Semgrep + CodeQL, G2.5 Trivy, G2.3 Playwright; not bundled
...
├── .github/workflows/ci.yml    # L1: G1.8 Gitleaks, G1.6 oasdiff, format, lint, types, G1.9 Conventional Commits, tests+coverage, build, guardrails
...
`pnpm verify` is the L1 suite minus G1.8 and G1.6, plus G1.9. L1 CI installs Gitleaks and
oasdiff, runs `check:secrets`, then `check:contract`, then format/lint/types, then
`check:commits`, then tests+coverage, then `gen:minis-config`. G1.8 and G1.6 are not folded
into `verify` because they need the binaries; a missing binary is red in CI, not a skip.
G1.9 is folded into `verify` because it is a git log with no extra binary. History on
`main` is not rewritten. L2 (`check:licenses`, `check:migrate`, `check:integration`,
`check:artifact`, `check:sast`, `check:codeql`, `check:sca`, `check:smoke`) is a second
workflow and is not folded into `verify`, so adding it cannot become a reason to skip a
verify step.
