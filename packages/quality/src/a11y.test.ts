...
describe('isScreenFileName / listScreenFiles', () => {
  ...
  it('lists the play fixture next to home, browse, drama and fallback when all five are present', () => {
    const root = tempDir('a11y-walk-play-');
...
describe('missingRequiredScreenStems / toRepoFile / formatHit', () => {
  it('requires the SCR-02, SCR-03, SCR-04, SCR-05 and SCR-13 fixtures so deleting any is red', () => {
...
describe('runA11yCheck', () => {
  ...
  it('the committed SCR-02, SCR-03, SCR-04, SCR-05 and SCR-13 fixtures pass the real axe-core run in jsdom', async () => {
    const output = await runA11yCheck({
      root: repoRoot,
