...
<body>
  <main data-testid="profile-page">
    ...
    <a href="#/history">Continue watching</a>
  </main>
</body>
</html>
`;
...
describe('isScreenFileName / listScreenFiles', () => {
  ...
  it('lists the profile fixture next to home, browse, drama, play and fallback when all six are present', () => {
    const root = tempDir('a11y-walk-profile-');
...
describe('missingRequiredScreenStems / toRepoFile / formatHit', () => {
  it('requires the SCR-02, SCR-03, SCR-04, SCR-05, SCR-06 and SCR-13 fixtures so deleting any is red', () => {
...
describe('runA11yCheck', () => {
  ...
  it('the committed SCR-02, SCR-03, SCR-04, SCR-05, SCR-06 and SCR-13 fixtures pass the real axe-core run in jsdom', async () => {
    const output = await runA11yCheck({
      root: repoRoot,
    ...
    expect(output.exitCode).toBe(0);
    expect(output.stdout).toContain('a11y passed');
    expect(output.stdout).toContain('6 screens');
    expect(output.stdout).toContain(A11Y_HOST_DISCLAIMER);
  });
});
...
describe('QA-010 does not skip the engine or claim TikTok WebView', () => {
  ...
  it('the SCR-06 fixture still matches ProfilePage structure', () => {
    ...
    expect(page).toMatch(/data-testid="profile-sign-in"|profile.signInPrompt/);
    expect(page).toMatch(/data-testid="profile-entries"/);
    expect(page).toMatch(/data-testid="history-entry"/);
    expect(page).toMatch(/data-testid="favorites-entry"/);
    expect(page).toMatch(/data-testid="wallet-entry"/);
    expect(page).toMatch(/data-testid="settings-entry"/);
    const fixture = readFileSync(
    ...
    expect(fixture).toContain('data-testid="history-entry"');
    expect(fixture).toContain('data-testid="favorites-entry"');
    expect(fixture).toContain('data-testid="wallet-entry"');
    expect(fixture).toContain('data-testid="settings-entry"');
    expect(fixture).toContain('<html lang="en">');
    expect(fixture).toContain('Me');
    expect(fixture).toContain('#/history');
    expect(fixture).not.toMatch(/<video|<audio|<iframe/i);
    const routes = readFileSync(join(repoRoot, 'app/src/routes/routes.ts'), 'utf8');
    expect(routes).toMatch(/me: '\/me'/);
  });
});
