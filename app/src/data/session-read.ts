...
/**
 * What a failed *session-scoped* read means, which is a distinction the anonymous reads never have
 * to draw.
 *
 * "Your list is empty" is three different products, and the value of every personal screen depends
 * on telling them apart:
 *
 * | Server says | Screen shows | Why it must be its own state |
 * | --- | --- | --- |
 * | `200` with an empty collection | "nothing here yet" + a way to get some | The viewer is known and has nothing. The action is to go and get something |
 * | `401` | "sign in to see this" + retry silent login | The viewer is **not known**. There may well be a list; we are not allowed to see it. Rendering this as empty tells the viewer their data was lost |
 ...
 * others are legitimately "an empty screen". A viewer who has followed twenty dramas and is shown
 * "nothing here yet" because a token was missing has been told their list is gone. They do not know
...
/**
 * Statuses that mean "this endpoint is not deployed" rather than "your data is missing".
 *
 * A collection endpoint cannot answer `404` about the caller's own data: either the route exists and
 * the collection is empty, or the route does not exist. `405` is the same fact reported by a proxy
 * that knows the path but not the method, and `501` is a gateway saying it outright.
 *
 * They degrade to the empty state rather than to an error because an error state asks the viewer to
 * do something about our missing feature.
 */
export const UNAVAILABLE_STATUSES: readonly number[] = [404, 405, 501];

export function presentSessionReadFailure(failure: ApiFailure): SessionReadPresentation {
