 ...
 *
 * The endpoint is unauthenticated by definition, which is why this takes an `HttpClient` built
 * without a token source. A session cannot be created by presenting a session, and a stale bearer
 * on this request is a dead credential travelling for no reason.
 */
...
export function createSessionApi(http: HttpClient): SessionApi {
  return {
