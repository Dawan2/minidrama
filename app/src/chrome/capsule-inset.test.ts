...
describe('capsuleInsetFromRect', () => {
  ...
  it('keeps the conservative default when the platform sent no usable rectangle', () => {
    ...
    expect(capsuleInsetFromRect(rect({ height: 0, top: 8, bottom: 8 }), VIEWPORT)).toEqual(
      fallback,
    );
    expect(capsuleInsetFromRect(rect({ left: VIEWPORT, right: VIEWPORT + 88 }), VIEWPORT)).toEqual(
      fallback,
    );
  ...
  it('does not treat a missing rectangle as zero, which would sit under the capsule', () => {
    const missing = capsuleInsetFromRect(
      rect({ width: 0, height: 0, left: 0, right: 0 }),
      VIEWPORT,
    );
